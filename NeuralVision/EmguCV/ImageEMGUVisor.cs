...
namespace Mouse_Hunter.NeuralVision.EmguCV
{
    public class ImageEMGUVisor
    {
        ...
        public VisionArea WatchArea(bool isInputMutated, SearchArgsWrapper wrap, ref VisionArea area)
        {
            ...
            Image<Bgr, byte> imgForMaskNSearch = repos.GetEmguImage(sourcePath);
            Image<Bgr, byte> imageToBitmap = imgForMaskNSearch.Copy();
            Image<Bgr, byte> imageToDrawRectNSave = imgForMaskNSearch.Copy();
            ...
            area = GetManyOccurencies(aimTitle, ref template, ref imgForMaskNSearch, wrap.threshhold, area);
            imgForMaskNSearch.Dispose();
            template.Dispose();
...
/*            area.OriginAimCoords = allPositions[0];
            ...
            area.AreaBitmap = imageToBitmap.ToBitmap();

            //Draw all on screen
            foreach (Rectangle matchRect in allPositions)
            {
                imageToDrawRectNSave.Draw(matchRect, new Bgr(System.Drawing.Color.Red), 3);
            }
            repos.SaveEmguImage(imageToDrawRectNSave, savePath);
            imageToDrawRectNSave.Dispose();
            return area;
        }

        private VisionArea GetManyOccurencies(string aimTitle, ref Image<Bgr, byte> template, ref Image<Bgr, byte> source,
            double threshhold, VisionArea visionArea, int counter = 0)
        {
            using (Image<Gray, float> result = source.MatchTemplate(template, TemplateMatchingType.CcoeffNormed))
            {
                ...
                if (maxValues[0] >= threshhold && counter < 10)
                {
                    ...
                    visionArea = GetManyOccurencies(aimTitle, ref template, ref source, threshhold, visionArea, counter);
                    // do stuff with match
                    // etc..
                }
                return visionArea;
