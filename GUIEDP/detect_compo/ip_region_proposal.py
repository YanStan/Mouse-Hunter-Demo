...
def compo_detection(input_img_path, output_root, uied_params, classifier=None, show=False, wai_key=0):
    ...
    Compo.compos_update(uicompos, org.shape)
    #draw.draw_bounding_box(org, uicompos, show=show, name='merged compo', write_path=pjoin(ip_root, name + '.jpg'),
                           #wait_key=wai_key)
    ...
    box_coords = []
    if(uicompos):
        box_coords = file.save_corners_json(pjoin(ip_root, name + '.json'), uicompos)

    print("[Compo Detection Completed in %.3f s] Input: %s Output: %s" % (
    time.process_time() - start, input_img_path, pjoin(ip_root, name + '.json')))
    return box_coords
